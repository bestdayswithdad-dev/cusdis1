...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  // --- GET LOGIC: Now supports both Blog (specific page) and Dashboard (all) ---
  if (req.method === 'GET') {
    const { pageId } = req.query
    
    try {
      // If pageId is provided, filter by it (Blog view)
      // If pageId is missing, fetch everything for this project (Dashboard view)
      ...
      if (pageId) {
        ...
        whereClause.approved = true; // Blog only sees approved
      }

      const comments = await prisma.comment.findMany({
        where: whereClause,
        include: { Page: true }, // Ensure we include the Page object for your tabs
        orderBy: { created_at: 'desc' } // Newest first for the Dashboard
      })
