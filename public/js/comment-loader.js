(function() {
    ...
    // 3. UI STYLING: Match Comment Bubble + Compact Embedded Input
    const styling = `
    <style>
        ...
        /* THE FORM CONTAINER: Matches .comment-card style */
        #comment-form {
        ...
        /* Standardized styles for nickname and text area */
        ...
        #nickname.embedded-input-field {
            width: 150px; /* Compact nickname field */
            border-right: 1px solid rgba(0,0,0,0.08) !important;
            flex-shrink: 0;
        }

        #comment-body.embedded-input-field {
            flex-grow: 1;
            resize: none;
            padding-right: 70px; /* Room for embedded button */
        }

        /* COMPACT, EMBEDDED SUBMIT BUTTON */
        .embedded-submit-btn {
            ...
            font-weight: 800;
            cursor: pointer;
            width: 60px; /* Smaller, compact button */
        ...
        .embedded-submit-btn:hover {
            background: #2563eb !important; /* Lighter/Active Blue */
        }

        /* AUTH & BADGE STYLE RESTORED */
        .auth-bar {
            display: flex; flex-direction: column; align-items: center; gap: 12px;
            margin-bottom: 25px; padding: 20px; background: #fff !important;
            border-radius: 8px; box-shadow: 0 0 10px rgba(0,0,0,0.05);
            width: fit-content; margin-left: auto; margin-right: auto;
        }
        .auth-bar.is-logged-in { background: rgba(5, 150, 105, 0.05) !important; border: 1px solid rgba(5, 150, 105, 0.2); }
        ...
        .comment-disclaimer { font-size: 10px; color: #64748b; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; margin-top: 10px; }
    ...
    // 6. MAIN RENDER: Janitor Integrated
    const render = async () => {
        ...
        try {
            ...
            // AUTH BAR: (Correct Privacy Policy URL Restore)
            let authBarHtml = `
            ...
            container.innerHTML = styling + `
                <div>
                    <div id="comment-form">
                        ...
                        <div class="embedded-input-container">
                            <input type="text" id="nickname" class="embedded-input-field" placeholder="Nickname" value="${currentUser?.user?.user_metadata?.full_name || ''}" />
                            <textarea id="comment-body" class="embedded-input-field" placeholder="Share your experience..." rows="3"></textarea>
                            <button class="embedded-submit-btn" onclick="window.submitReview()">POST</button>
                        </div>
    ...
    window.submitReview = async function() { 
        ...
        const res = await fetch('https://cusdis-jet-one.vercel.app/api/public-comments', { 
            ...
            body: JSON.stringify({ content, nickname, pageId: getCleanUrl(), pageTitle: document.title.split(' : ')[0], parentId: parentId || null, by_email: email }) 
        }); 
    ...
    // 8. AUTH HANDLERS: Restore Janitor timeout Fix
