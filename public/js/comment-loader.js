(function() {
    ...
    const createCommentHtml = (comment) => {
        ...
        return `
            <div class="comment-content-wrapper">
                ...
                <div class="comment-text-row"><p>${comment.content}</p></div>
        ...
        let html = `
            <div style="margin-top: 0;">
                ...
                <div id="comment-form" style="margin-bottom: 30px; text-align: center;">
                    <div id="reply-indicator" style="display:none; background:#e0f2fe; color:#0369a1; padding:10px; border-radius:6px; font-size:11px; font-weight:700; margin-bottom:10px; text-align:left; cursor:pointer;" onclick="window.cancelReply()">Replying (Click to cancel X)</div>
    ...
    window.handleLikeAction = async (id, liked) => { if (!currentUser) { alert("Join!"); return; } const rpc = liked ? 'handle_remove_like' : 'handle_new_like'; await window.supabaseClient.rpc(rpc, { c_id: String(id), u_id: currentUser.id }); render(); };
